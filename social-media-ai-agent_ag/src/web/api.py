...
async def get_all_instruments():
    ...
    return universe_manager.get_all_instruments()


@app.post("/api/universe/refresh")
async def refresh_universe():
    """Refresh the universe and return refreshed instruments."""
    refreshed_instruments = universe_manager.refresh_universe()
    return refreshed_instruments


@app.post("/api/features/extract", response_model=dict)
async def extract_features(request: SymbolRequest):
...
async def get_post_status(thread_id: str):
    ...
    try:
        ...
        return PostResponse(
            ...
            char_count=post_content.char_count
            if post_content and hasattr(post_content, "char_count")
            else None,
